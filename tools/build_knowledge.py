...
def main() -> None:
    ...
    # Prune orphans: stale auto-generated sections from renamed/removed JSON
    # sources would otherwise linger and get Read as live knowledge. Only delete
    # files we recognize as our own output (carrying the auto-generated marker).
    removed = 0
    for stale in ondemand_dir.glob("*.md"):
        if stale.name in written:
            continue
        try:
            head = stale.read_text(encoding="utf-8")[:120]
        except OSError:
            continue
        if "Auto-generated by tools/build_knowledge.py" in head:
            stale.unlink()
            removed += 1
            print(f"  Pruned orphan {stale.relative_to(REPO_ROOT)}")

    print(f"Wrote {len(ondemand)} on-demand sections under "
          f"{ondemand_dir.relative_to(REPO_ROOT)}/"
          + (f" ({removed} orphan(s) pruned)" if removed else ""))
