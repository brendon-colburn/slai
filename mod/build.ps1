<#
.SYNOPSIS
    Builds the SLAI mod DLL.

.DESCRIPTION
    Compiles SLAI.dll against the game's assemblies. Does NOT install
...
Or set it once in your PowerShell profile:
  `$env:STS2_GAME_DIR = "C:\Steam\steamapps\common\Slay the Spire 2"
"@ -ForegroundColor Red
    exit 1
}
...
$project = Join-Path $scriptDir "SLAI.csproj"
$outDir = Join-Path (Join-Path $scriptDir "out") "SLAI"

Write-Host "=== Building SLAI mod ($Configuration) ===" -ForegroundColor Cyan
...
Write-Host "  $outDir\SLAI.dll"
Write-Host "  $scriptDir\mod_manifest.json  ->  mods\SLAI.json"
