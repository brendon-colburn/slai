...
public static partial class McpMod
{
    private static Dictionary<string, object?> BuildGameState()
    {
        ...
        if (!RunManager.Instance.IsInProgress)
        {
            ...
            if (tree?.Root != null)
            {
                ...
                // Multiplayer Load lobby â€” resume saved MP run. Pushed by NMultiplayerSubmenu
        ...
        NCharacterSelectScreen charSelect)
    {
        ...
        // MP lobby block â€” surfaces roster / ready state / ascension when this character
    ...
    private static Dictionary<string, object?> BuildStartRunLobbyState(StartRunLobby lobby)
    {
        ...
        foreach (var p in lobby.Players)
        {
            var entry = new Dictionary<string, object?>
            {
                ...
                // when hosting â€” clients can infer host-ness by player_id when needed.
    ...
    private static Dictionary<string, object?> BuildPlayerState(Player player)
    {
        ...
        // Master deck: the full run deck (player.Deck is a CardPile with PileType.Deck).
        // Exposed on every screen, not just during combat â€” coaching tools, deck trackers,
        // and post-combat analysis all need this to function between fights. The combat
        // hand/draw/discard/exhaust piles above are a subset of this during combat.
        //
        // Uses BuildCardInfo (rich shape: id/type/rarity/is_upgraded/keywords) rather
        // than BuildPileCardList (lightweight shape: name/cost/description only) because
        // analysis tools need to classify cards by type and detect upgrades.
        try
        {
            ...
            if (deckCards != null)
            {
                ...
                foreach (var card in deckCards)
                    list.Add(BuildCardInfo(card, PileType.Deck));
                state["master_deck"] = list;
                state["master_deck_count"] = deckCards.Count;
            }
        }
        catch (Exception ex)
        {
            Godot.GD.PrintErr($"[SLAI] Failed to serialize master_deck: {ex.GetType().Name}: {ex.Message}");
        }
    ...
    private static Dictionary<string, object?> BuildMapState(RunState runState)
    {
        ...
        // Boss identity comes from the live act's EncounterModel â€” BossEncounter
        // throws if the act hasn't finished setup yet, so guard the access.
